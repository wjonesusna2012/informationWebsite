...
const SelectedStoryAccoridan = ({title, description}: {title: string, description: string}) => {
  return (
    <Accordion>
...
const SelectedStories = ({selectedStories}: {selectedStories: Array<TopicCardProps>}) => {
  return (
    <>
      {selectedStories.map(sS => {
        return <SelectedStoryAccoridan title={sS.postTitle} description={sS.summary} />
      })}
    </>
  )
} 
